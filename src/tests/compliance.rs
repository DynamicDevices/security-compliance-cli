...
impl SecurityTest for ComplianceTests {
    async fn run(&self, target: &mut Target) -> Result<TestResult> {
        ...
        let result = match self {
            ...
            Self::CraVulnerabilityManagement => {
                self.test_cra_vulnerability_management(target).await
            }
...
impl ComplianceTests {
    async fn test_cra_data_protection(
        &self,
        target: &mut Target,
        ...
        details.push(format!(
            "LUKS encryption: {}",
            !luks_check.stdout.is_empty()
        ));

        // Check secure communications (TLS)
        let tls_check = target
            .execute_command("openssl version 2>/dev/null || echo 'not_available'")
            .await?;
        ...
        let access_control = target
            .execute_command("ls -la /etc/passwd /etc/shadow | grep -E '^-r--------'")
            .await?;
        ...
        details.push(format!(
            "Access controls: {}",
            !access_control.stdout.is_empty()
        ));

        // Check secure boot (integrity protection)
        let secure_boot = target
            .execute_command("dmesg | grep -i 'ahab\\|secure.*boot'")
            .await?;
        ...
        details.push(format!(
            "Secure boot: {}",
            secure_boot.stdout.contains("AHAB")
        ));

        let compliance_score = compliance_items.len();
        let details_str = format!(
            "CRA Article 11 compliance items: {:?}\n{}",
            compliance_items,
            details.join("\n")
        );
        ...
        if compliance_score >= 3 {
            Ok((
                TestStatus::Passed,
                format!(
                    "CRA data protection compliant ({}/4 items)",
                    compliance_score
                ),
                Some(details_str),
            ))
            ...
            Ok((
                TestStatus::Warning,
                format!("Partial CRA compliance ({}/4 items)", compliance_score),
                Some(details_str),
            ))
        } else {
            Ok((
                TestStatus::Failed,
                format!("CRA non-compliant ({}/4 items)", compliance_score),
                Some(details_str),
            ))
        }
    }

    async fn test_cra_vulnerability_management(
        &self,
        target: &mut Target,
    ) -> Result<(TestStatus, String, Option<String>)> {
        // Check for update mechanisms
        let ostree_check = target
            .execute_command("ostree admin status 2>/dev/null || echo 'not_available'")
            .await?;
        let aktualizr_check = target
            .execute_command("systemctl is-active aktualizr-lite 2>/dev/null || echo 'not_active'")
            .await?;

        // Check for vulnerability scanning tools
        let vuln_tools = target
            .execute_command("which nmap 2>/dev/null || which lynis 2>/dev/null || echo 'none'")
            .await?;
        ...
        let details = format!(
            "Update mechanism: {:?}\nOTA status: {}\nVuln tools: {}\nLast update: {}",
            compliance_features,
            aktualizr_check.stdout.trim(),
            vuln_tools.stdout.trim(),
            last_update.stdout.trim()
        );

        if compliance_features.len() >= 2 {
            Ok((
                TestStatus::Passed,
                "CRA vulnerability management capable".to_string(),
                Some(details),
            ))
        } else if !compliance_features.is_empty() {
            Ok((
                TestStatus::Warning,
                "Limited vulnerability management".to_string(),
                Some(details),
            ))
        } else {
            Ok((
                TestStatus::Failed,
                "No vulnerability management detected".to_string(),
                Some(details),
            ))
        }
    }

    async fn test_red_security_requirements(
        &self,
        target: &mut Target,
    ) -> Result<(TestStatus, String, Option<String>)> {
        ...
        let auth_check = target
            .execute_command("cat /etc/pam.d/common-auth 2>/dev/null | grep -v '^#' | head -3")
            .await?;
        ...
        details.push(format!(
            ...
            !auth_check.stdout.is_empty()
        ));

        // Check access control
        let access_check = target
            .execute_command("getfacl /etc/shadow 2>/dev/null | grep -E 'user::|group::'")
            .await?;
        if !access_check.stdout.is_empty() {
            security_measures.push("Access Control");
        }
        details.push("Access control: configured".to_string());

        // Check secure communications
        let secure_comm = target
            .execute_command("netstat -tuln | grep ':22\\|:443\\|:993'")
            .await?;
        ...
        details.push(format!(
            "Secure ports: {}",
            secure_comm.stdout.lines().count()
        ));

        let measure_count = security_measures.len();
        let details_str = format!(
            "RED 3.3 security measures: {:?}\n{}",
            security_measures,
            details.join("\n")
        );
        ...
        if measure_count >= 3 {
            Ok((
                TestStatus::Passed,
                format!("RED security compliant ({}/4 measures)", measure_count),
                Some(details_str),
            ))
        } else if measure_count >= 2 {
            Ok((
                TestStatus::Warning,
                format!("Partial RED compliance ({}/4 measures)", measure_count),
                Some(details_str),
            ))
        } else {
            Ok((
                TestStatus::Failed,
                format!("RED non-compliant ({}/4 measures)", measure_count),
                Some(details_str),
            ))
        }
    }
    ...
    async fn test_incident_response(
        ...
        let monitoring = target
            .execute_command(
                "which journalctl 2>/dev/null && echo 'systemd-journal' || echo 'no_journal'",
            )
            .await?;
        ...
        let netmon = target
            .execute_command(
                "which tcpdump 2>/dev/null || which netstat 2>/dev/null || echo 'no_netmon'",
            )
            .await?;
        ...
        let details = format!(
            "Logging: {}\nMonitoring: {}\nNetwork tools: {}\nIncident tools: {}",
            syslog_check.stdout.trim(),
            monitoring.stdout.contains("systemd-journal"),
            !netmon.stdout.contains("no_netmon"),
            tool_count
        );
        ...
        if capabilities.len() >= 3 {
            Ok((
                TestStatus::Passed,
                "Incident response capability good".to_string(),
                Some(details),
            ))
        } else if capabilities.len() >= 2 {
            Ok((
                TestStatus::Warning,
                "Basic incident response capability".to_string(),
                Some(details),
            ))
        } else {
            Ok((
                TestStatus::Failed,
                "Limited incident response capability".to_string(),
                Some(details),
            ))
        }
    }
    ...
    async fn test_audit_logging(
        ...
        let journal_size = target
            .execute_command(
                "journalctl --disk-usage 2>/dev/null | grep -o '[0-9.]*[KMGT]B' || echo '0B'",
            )
            .await?;

        // Check log rotation
        let logrotate = target
            .execute_command("ls -la /etc/logrotate.d/ | wc -l")
            .await?;

        // Check security-related logs
        let security_logs = target
            .execute_command(
                "journalctl --since='1 hour ago' | grep -i 'security\\|auth\\|fail' | wc -l",
            )
            .await?;
        ...
        if audit_features.len() >= 3 {
            Ok((
                TestStatus::Passed,
                format!("Audit logging comprehensive: {:?}", audit_features),
                Some(details),
            ))
        } else if audit_features.len() >= 2 {
            Ok((
                TestStatus::Warning,
                format!("Basic audit logging: {:?}", audit_features),
                Some(details),
            ))
        } else {
            Ok((
