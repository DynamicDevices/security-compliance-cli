...
impl RuntimeSecurityTests {
    ...
    async fn test_firewall_active(&self, target: &mut Target) -> Result<(TestStatus, String, Option<String>)> {
        ...
        details.push(format!("iptables binary available: {}", iptables_available.exit_code == 0));
        if used_sudo {
            details.push("Used sudo to access iptables rules".to_string());
        }
        
        // Check if we have any firewall rules configured
        if iptables_result.exit_code == 0 && !iptables_result.stdout.contains("Chain INPUT (policy ACCEPT)") {
            Ok((TestStatus::Passed, "Firewall rules configured".to_string(), Some(details.join("\n"))))
        } else if iptables_result.exit_code == 0 && iptables_result.stdout.contains("Chain INPUT (policy ACCEPT)") {
            // iptables is available but using default ACCEPT policy
            let rule_count = iptables_result.stdout.lines().filter(|line| 
                line.contains("ACCEPT") || line.contains("DROP") || line.contains("REJECT")
            ).count();
            
            if rule_count > 3 { // More than just the default chains
                Ok((TestStatus::Warning, "iptables available with some rules but default ACCEPT policy".to_string(), Some(details.join("\n"))))
            } else {
                // Default configuration - treat as warning in pre-production, but note that iptables is present
                Ok((TestStatus::Warning, "iptables present but firewall not configured (default ACCEPT policy)".to_string(), Some(details.join("\n"))))
            }
        } else if iptables_available.exit_code == 0 {
            // iptables binary exists but both direct and sudo commands failed
            if iptables_result.stderr.contains("Permission denied") || iptables_result.stderr.contains("Operation not permitted") {
                Ok((TestStatus::Warning, "iptables available but access denied".to_string(), Some(details.join("\n"))))
            } else {
                Ok((TestStatus::Warning, "iptables available but unable to read rules".to_string(), Some(details.join("\n"))))
            }
        } else {
            // No iptables at all - warning for pre-production environments
            Ok((TestStatus::Warning, "No firewall configuration detected (iptables not available)".to_string(), Some(details.join("\n"))))
        }
    }
    ...
    async fn test_ssh_configuration(&self, target: &mut Target) -> Result<(TestStatus, String, Option<String>)> {
        ...
        if ssh_config.stdout.contains("PermitRootLogin yes") {
            ...
            critical_issues.push("Root login permitted - this is a critical security vulnerability");
        ...
        if ssh_config.stdout.contains("PasswordAuthentication yes") {
            ...
            if current_user.stdout.trim() == "fio" {
                ...
                let ssh_key_check = target.execute_command("ls -la ~/.ssh/authorized_keys 2>/dev/null || echo 'no_keys'").await?;
                let home_setup_check = target.execute_command("ls -la ~/.bashrc ~/.profile 2>/dev/null | wc -l").await?;
                ...
                if ssh_key_check.stdout.contains("no_keys") && 
                   home_setup_check.stdout.trim().parse::<i32>().unwrap_or(0) <= 1 {
                    ...
                    security_issues.push("Default user with password auth (critical security risk)");
                } else {
            ...
            if ssh_algorithms.stdout.contains("password") || !ssh_algorithms.stdout.contains("algorithms_not_available") {
                security_issues.push("Password authentication status unclear (may be enabled by default)");
            }
        }
        ...
        if !ssh_algorithms.stdout.contains("algorithms_not_available") {
            ...
            if ssh_algorithms.stdout.contains("diffie-hellman-group1-sha1") ||
               ssh_algorithms.stdout.contains("diffie-hellman-group14-sha1") ||
               ssh_algorithms.stdout.contains("diffie-hellman-group-exchange-sha1") {
            ...
            if ssh_algorithms.stdout.contains("chacha20-poly1305@openssh.com") ||
            ...
            if ssh_algorithms.stdout.contains("umac-128-etm@openssh.com") ||
               ssh_algorithms.stdout.contains("hmac-sha2-256-etm@openssh.com") ||
               ssh_algorithms.stdout.contains("hmac-sha2-512-etm@openssh.com") {
            ...
            if ssh_algorithms.stdout.contains("curve25519-sha256") ||
               ...
               ssh_algorithms.stdout.contains("diffie-hellman-group16-sha512") {
        ...
        if !critical_issues.is_empty() {
            Ok((TestStatus::Failed, format!("SSH critical security issues: {}", critical_issues.join(", ")), Some(details)))
    ...
    async fn test_user_permissions(&self, target: &mut Target) -> Result<(TestStatus, String, Option<String>)> {
        ...
        let root_exists = target.execute_command("id root 2>/dev/null || echo 'not_found'").await?;
        
        // Check if root login is disabled
        let root_passwd_entry = target.execute_command("grep '^root:' /etc/passwd").await?;
        let root_shadow_entry = target.execute_command("grep '^root:' /etc/shadow 2>/dev/null || echo 'shadow_not_accessible'").await?;
        ...
        if using_default_creds {
            ...
            let fio_shadow_entry = target.execute_command("grep '^fio:' /etc/shadow 2>/dev/null || echo 'shadow_not_accessible'").await?;
            
            // Check if password might be default by looking at shadow file
            if !fio_shadow_entry.stdout.contains("shadow_not_accessible") && !fio_shadow_entry.stdout.trim().is_empty() {
                ...
                if shadow_fields.len() >= 2 {
                    ...
                    if password_hash.len() < 20 || password_hash.starts_with("$1$") {
                        ...
                        details.push("WARNING: fio user may be using default or weak password".to_string());
                    }
                }
            }
            
            // Additional checks for default setup indicators
            let home_dir_check = target.execute_command("ls -la /home/fio/.bashrc /home/fio/.profile 2>/dev/null | wc -l").await?;
            let ssh_key_check = target.execute_command("ls -la /home/fio/.ssh/authorized_keys 2>/dev/null || echo 'no_keys'").await?;
            
            if home_dir_check.stdout.trim().parse::<i32>().unwrap_or(0) <= 1 && ssh_key_check.stdout.contains("no_keys") {
                // Minimal home directory setup + no SSH keys suggests default installation
                default_creds_risk = true;
                details.push("WARNING: fio user appears to have default installation setup".to_string());
                security_issues.push("Default user 'fio' detected with potentially default credentials".to_string());
            }
        }
        
        details.push(format!("Root users (UID 0): {}", if root_users.stdout.trim().is_empty() { "none detected" } else { &root_users.stdout }));
        details.push(format!("Root user exists: {}", if root_exists.stdout.contains("not_found") { "no" } else { "yes" }));
        details.push(format!("Shell users count: {}", shell_users.stdout.trim()));
        // Add default credentials information to details
        if using_default_creds {
            details.push(format!("Current user: {} (default Foundries.io user)", current_user.stdout.trim()));
            if default_creds_risk {
                details.push("⚠️  WARNING: Using default 'fio' user with potentially default credentials".to_string());
            } else {
        ...
        details.push(format!("Current user groups: {}", current_user_groups.stdout.trim()));
        ...
        if !root_passwd_entry.stdout.trim().is_empty() {
            ...
            if passwd_fields.len() >= 7 {
                ...
                if shell.contains("/nologin") || shell.contains("/false") || shell.contains("/bin/false") {
                ...
                if password_field == "x" {
                    ...
                    if !root_shadow_entry.stdout.contains("shadow_not_accessible") && !root_shadow_entry.stdout.trim().is_empty() {
                        let shadow_fields: Vec<&str> = root_shadow_entry.stdout.trim().split(':').collect();
                        if shadow_fields.len() >= 2 {
                            let password_hash = shadow_fields[1];
                            if password_hash == "!" || password_hash == "*" || password_hash.starts_with("!") {
                                ...
                                if !root_login_disabled {
                                    security_issues.push("Root user password is set and login shell is available".to_string());
                                }
                            } else {
                                details.push("Root password: empty (no password)".to_string());
                                if !root_login_disabled {
                                    security_issues.push("Root user has no password and login shell is available".to_string());
                                }
                            }
                        }
                    } else {
                        details.push("Root password: shadow file not accessible for verification".to_string());
                    }
                    ...
                    if !root_login_disabled {
                        security_issues.push("Root user has password set in passwd file and login shell available".to_string());
                    }
                }
            }
        } else {
            ...
            if !root_exists.stdout.contains("not_found") {
                ...
                details.push("Root user: exists but not in passwd file (possibly managed by system)".to_string());
                ...
                if !passwd_root_check.stdout.trim().is_empty() {
                    let passwd_fields: Vec<&str> = passwd_root_check.stdout.trim().split(':').collect();
                    if passwd_fields.len() >= 7 {
                        let shell = passwd_fields[6];
                        if shell.contains("/nologin") || shell.contains("/false") || shell.contains("/bin/false") {
                            root_login_disabled = true;
                            details.push(format!("Root shell (via getent): {} (login disabled)", shell));
                        } else {
                            details.push(format!("Root shell (via getent): {} (login possible)", shell));
                            security_issues.push("Root user has login shell enabled".to_string());
                        }
                    }
                } else {
        ...
        let root_count = if root_users.stdout.trim().is_empty() {
            ...
            if !root_exists.stdout.contains("not_found") { 1 } else { 0 }
        } else {
            root_users.stdout.lines().filter(|line| !line.trim().is_empty()).count()
        };
        ...
        if !security_issues.is_empty() {
            Ok((TestStatus::Failed, format!("User permission security issues: {}", security_issues.join(", ")), Some(details.join("\n"))))
        } else if root_login_disabled && shell_count <= 3 {
            // Root login is properly disabled and reasonable number of shell users
            Ok((TestStatus::Passed, "User permissions secure (root login disabled)".to_string(), Some(details.join("\n"))))
            ...
            Ok((TestStatus::Warning, "Unable to verify root user configuration".to_string(), Some(details.join("\n"))))
        } else {
    ...
    async fn test_kernel_protections(&self, target: &mut Target) -> Result<(TestStatus, String, Option<String>)> {
        ...
        if arch.stdout.trim() == "aarch64" {
            ...
            let pan_check = target.execute_command("grep -i 'pan' /proc/cpuinfo || echo 'not_found'").await?;
        ...
        if kptr.stdout.trim() == "2" {
            ...
            recommendations.push("Enable kernel pointer restriction: echo 1 > /proc/sys/kernel/kptr_restrict");
        }
        ...
        if dmesg_restrict.stdout.trim() == "1" {
            ...
            recommendations.push("Enable dmesg restriction: echo 1 > /proc/sys/kernel/dmesg_restrict");
        }
        ...
        if arch.stdout.trim() == "aarch64" {
            ...
            let pauth_check = target.execute_command("grep -i 'paca\\|pacg' /proc/cpuinfo || echo 'not_found'").await?;
            ...
            let smep_smap = target.execute_command("grep -i 'smep\\|smap' /proc/cpuinfo || echo 'not_found'").await?;
        ...
        if !recommendations.is_empty() {
            ...
            if recommendations.iter().any(|r| r.contains("randomize_va_space")) {
        ...
        if protection_count >= 3 {
            ...
            Ok((TestStatus::Failed, "Insufficient kernel protections detected".to_string(), Some(details_str)))
        }
    }

    async fn test_readonly_filesystem(&self, target: &mut Target) -> Result<(TestStatus, String, Option<String>)> {
        let mut details = Vec::new();
        ...
        let lmp_check = target.execute_command("cat /etc/os-release | grep -i 'linux.*micro.*platform\\|foundries'").await?;
        let is_lmp = !lmp_check.stdout.is_empty();

        // Check mount points and their read-only status
        let mounts = target.execute_command("mount | grep -E '^/dev|^overlay|^tmpfs'").await?;
        ...
        for dir in &critical_dirs {
            let mount_check = target.execute_command(&format!("findmnt -n -o OPTIONS {} 2>/dev/null | grep -o 'ro\\|rw' | head -1", dir)).await?;
            ...
            if mount_status == "ro" {
                ...
                if *dir == "/" && is_lmp {
                    ...
                    let ostree_check = target.execute_command("ostree --version 2>/dev/null").await?;
                    if !ostree_check.stdout.is_empty() {
                        ...
                        // Check for overlay
                        let overlay_check = target.execute_command("mount | grep 'overlay on /'").await?;
                        if !overlay_check.stdout.is_empty() {
                            ...
                            issues.push(format!("{} is mounted read-write (should be read-only)", dir));
                        }
                    }
                } else if *dir == "/boot" && is_lmp {
                    // Boot partition might need to be RW for OTA updates in LMP
                    details.push("Boot partition is read-write (may be needed for OTA updates)".to_string());
        ...
        for dir in &writable_dirs {
            ...
            if mount_status == "rw" {
                ...
                issues.push(format!("{} is read-only (should be writable for proper operation)", dir));
            }
        }
        ...
        if is_lmp {
            ...
            let ostree_check = target.execute_command("ostree --version 2>/dev/null || echo 'not_found'").await?;
            ...
            let persistent_check = target.execute_command("ls -la /var/sota /var/lib 2>/dev/null | head -5").await?;
        ...
        let write_test = target.execute_command("touch /usr/test_readonly_check 2>&1 || echo 'write_blocked'").await?;
        if write_test.stdout.contains("write_blocked") || write_test.stdout.contains("Read-only") {
            ...
            let _ = target.execute_command("rm -f /usr/test_readonly_check").await;
        }
        ...
        if readonly_count >= 3 && issue_count == 0 {
            Ok((TestStatus::Passed, format!("Read-only filesystem properly configured ({} protected areas)", readonly_count), details_str))
        } else if readonly_count >= 2 && issue_count <= 1 {
            Ok((TestStatus::Warning, format!("Mostly read-only filesystem ({} protected, {} issues)", readonly_count, issue_count), details_str))
        } else {
            Ok((TestStatus::Warning, format!("Insufficient read-only protection ({} protected, {} issues)", readonly_count, issue_count), details_str))
        }
    }
}
