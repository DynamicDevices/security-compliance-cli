...
impl ContainerSecurityTests {
    async fn test_docker_security_config(
        &self,
        target: &mut Target,
    ) -> Result<(TestStatus, String, Option<String>)> {
        // Check if Docker/Podman is installed
        let docker_check = target
            .execute_command(
                "which docker 2>/dev/null || which podman 2>/dev/null || echo 'not_found'",
            )
            .await?;
        ...
        if docker_check.stdout.contains("not_found") {
            return Ok((
                TestStatus::Failed,
                "No container runtime detected - install Docker or Podman".to_string(),
                None,
            ));
        }
        ...
        let rootless_check = target
            .execute_command("docker info 2>/dev/null | grep -i rootless || echo 'not_rootless'")
            .await?;
        ...
        if security_features.len() >= 2 {
            Ok((
                TestStatus::Passed,
                format!(
                    "Container security configured ({} features)",
                    security_features.len()
                ),
                Some(details.join("\n")),
            ))
        } else if !security_features.is_empty() {
            Ok((
                TestStatus::Warning,
                format!(
                    "Basic container security ({} features)",
                    security_features.len()
                ),
                Some(details.join("\n")),
            ))
            ...
            Ok((
                TestStatus::Failed,
                "Insufficient container security configuration".to_string(),
                Some(details.join("\n")),
            ))
        }
    }

    async fn test_container_image_security(
        &self,
        target: &mut Target,
    ) -> Result<(TestStatus, String, Option<String>)> {
        ...
        if security_indicators.len() >= 2 {
            Ok((
                TestStatus::Passed,
                format!("Image security good: {:?}", security_indicators),
                Some(details),
            ))
        } else if !security_indicators.is_empty() {
            Ok((
                TestStatus::Warning,
                format!("Basic image security: {:?}", security_indicators),
                Some(details),
            ))
        } else {
            Ok((
                TestStatus::Failed,
                "Poor container image security".to_string(),
                Some(details),
            ))
        }
    }

    async fn test_runtime_security(
        &self,
        target: &mut Target,
    ) -> Result<(TestStatus, String, Option<String>)> {
        ...
        if privileged_check.stdout.trim() == "no_privileged"
            || privileged_check.stdout.trim().is_empty()
        {
        ...
        if runtime_security.len() >= 2 {
            Ok((
                TestStatus::Passed,
                "Container runtime security good".to_string(),
                Some(details),
            ))
        } else if !runtime_security.is_empty() {
            Ok((
                TestStatus::Warning,
                "Basic runtime security".to_string(),
                Some(details),
            ))
        } else {
            Ok((
                TestStatus::Failed,
                "Poor container runtime security".to_string(),
                Some(details),
            ))
        }
    }
    ...
    async fn test_network_isolation(
        ...
        let network_policies = target
            .execute_command("iptables -L DOCKER-USER 2>/dev/null | wc -l")
            .await?;

        let mut isolation_features = Vec::new();

        if !custom_networks.stdout.contains("no_custom")
            && !custom_networks.stdout.trim().is_empty()
        {
        ...
        if isolation_features.len() >= 2 {
            Ok((
                TestStatus::Passed,
                "Network isolation configured".to_string(),
                Some(details),
            ))
        } else if !isolation_features.is_empty() {
            Ok((
                TestStatus::Warning,
                "Basic network isolation".to_string(),
                Some(details),
            ))
        } else {
            Ok((
                TestStatus::Failed,
                "Poor network isolation".to_string(),
                Some(details),
            ))
        }
    }

    async fn test_user_namespaces(
        &self,
        target: &mut Target,
    ) -> Result<(TestStatus, String, Option<String>)> {
        // Check user namespace support
        let userns_check = target
            .execute_command(
                "docker info 2>/dev/null | grep -i 'user.*namespace' || echo 'no_userns'",
            )
            .await?;
        ...
        if namespace_security.len() >= 2 {
            Ok((
                TestStatus::Passed,
                "User namespace security configured".to_string(),
                Some(details),
            ))
        } else if !namespace_security.is_empty() {
            Ok((
                TestStatus::Warning,
                "Basic namespace security".to_string(),
                Some(details),
            ))
        } else {
            Ok((
                TestStatus::Failed,
                "Poor user namespace security".to_string(),
                Some(details),
            ))
        }
    }

    async fn test_selinux_contexts(
        &self,
        target: &mut Target,
    ) -> Result<(TestStatus, String, Option<String>)> {
        // Check SELinux status
        let selinux_status = target
            .execute_command("getenforce 2>/dev/null || echo 'no_selinux'")
            .await?;

        if selinux_status.stdout.contains("no_selinux")
            || selinux_status.stdout.trim() == "Disabled"
        {
            return Ok((
                TestStatus::Skipped,
                "SELinux not available or disabled".to_string(),
                None,
            ));
        }
        ...
        let selinux_policy = target
            .execute_command("sesearch -A -s container_t 2>/dev/null | wc -l")
            .await?;
        ...
        let details = format!(
            "SELinux: {}\nContexts: {}\nPolicy rules: {}",
            selinux_status.stdout.trim(),
            container_contexts.stdout,
            policy_rules
        );
        ...
        if selinux_features.len() >= 2 {
            Ok((
                TestStatus::Passed,
                "SELinux container security active".to_string(),
                Some(details),
            ))
        } else if !selinux_features.is_empty() {
            Ok((
                TestStatus::Warning,
                "Basic SELinux container security".to_string(),
                Some(details),
            ))
        } else {
            Ok((
                TestStatus::Failed,
                "Poor SELinux container security".to_string(),
                Some(details),
            ))
        }
    }

    async fn test_seccomp_profiles(
        &self,
        target: &mut Target,
    ) -> Result<(TestStatus, String, Option<String>)> {
        // Check seccomp support
        let seccomp_check = target
            .execute_command("docker info 2>/dev/null | grep -i seccomp || echo 'no_seccomp'")
            .await?;
        ...
        let custom_profiles = target
            .execute_command(
                "find /etc/docker /usr/share/containers -name '*seccomp*.json' 2>/dev/null | wc -l",
            )
            .await?;
        ...
        if seccomp_features.len() >= 2 {
            Ok((
                TestStatus::Passed,
                "Seccomp security profiles active".to_string(),
                Some(details),
            ))
        } else if !seccomp_features.is_empty() {
            Ok((
                TestStatus::Warning,
                "Basic seccomp security".to_string(),
                Some(details),
            ))
        } else {
            Ok((
                TestStatus::Failed,
                "Poor seccomp security".to_string(),
                Some(details),
            ))
        }
    }
