...
impl SecurityTest for ProductionTests {
    async fn run(&self, target: &mut Target) -> Result<TestResult> {
        ...
        let result = match self {
            ...
            Self::UnnecessaryServicesDisabled => {
                self.test_unnecessary_services_disabled(target).await
            }
...
impl ProductionTests {
    async fn test_debug_interfaces_disabled(
        &self,
        target: &mut Target,
        ...
        let debug_ports = target
            .execute_command(
                "netstat -tuln | grep -E ':1234|:4444|:8080|:9999' || echo 'no_debug_ports'",
            )
            .await?;

        // Check kernel debug options
        let kernel_debug = target
            .execute_command(
                "cat /proc/cmdline | grep -E 'debug|verbose|loglevel=8' || echo 'no_kernel_debug'",
            )
            .await?;

        // Check for development filesystems
        let dev_filesystems = target
            .execute_command("mount | grep -E 'debugfs|tracefs' || echo 'no_dev_fs'")
            .await?;
        ...
        if debug_issues.is_empty() {
            Ok((
                TestStatus::Passed,
                "Debug interfaces properly disabled".to_string(),
                Some(details),
            ))
        } else if debug_issues.len() <= 1 {
            Ok((
                TestStatus::Warning,
                format!("Some debug interfaces present: {:?}", debug_issues),
                Some(details),
            ))
        } else {
            Ok((
                TestStatus::Failed,
                format!("Multiple debug interfaces active: {:?}", debug_issues),
                Some(details),
            ))
        }
    }

    async fn test_development_tools_removed(
        &self,
        target: &mut Target,
    ) -> Result<(TestStatus, String, Option<String>)> {
        // Check for common development tools
        let dev_tools = [
            "gcc", "g++", "make", "cmake", "git", "vim", "emacs", "nano", "gdb", "strace",
            "valgrind",
        ];
        let mut found_tools = Vec::new();

        for tool in &dev_tools {
            let tool_check = target
                .execute_command(&format!("which {} 2>/dev/null || echo 'not_found'", tool))
                .await?;
        ...
        if found_tools.is_empty() && dev_pkg_count == 0 && src_dir_count == 0 {
            Ok((
                TestStatus::Passed,
                "Development tools properly removed".to_string(),
                Some(details),
            ))
        } else if found_tools.len() <= 2 && dev_pkg_count <= 2 {
            Ok((
                TestStatus::Warning,
                "Some development tools remain".to_string(),
                Some(details),
            ))
        } else {
            Ok((
                TestStatus::Failed,
                "Many development tools still present".to_string(),
                Some(details),
            ))
        }
    }

    async fn test_default_credentials_changed(
        &self,
        target: &mut Target,
    ) -> Result<(TestStatus, String, Option<String>)> {
        ...
        let no_password = target
            .execute_command(
                "awk -F: '$2 == \"\" { print $1 }' /etc/shadow 2>/dev/null | wc -l || echo '0'",
            )
            .await?;
        ...
        let ssh_host_keys = target
            .execute_command("ls -la /etc/ssh/ssh_host_*key* 2>/dev/null | wc -l")
            .await?;
        ...
        if credential_issues.is_empty() && ssh_key_count > 0 && host_key_count > 0 {
            Ok((
                TestStatus::Passed,
                "Default credentials properly changed".to_string(),
                Some(details),
            ))
        } else if credential_issues.len() <= 1 {
            Ok((
                TestStatus::Warning,
                "Some credential issues detected".to_string(),
                Some(details),
            ))
        } else {
            Ok((
                TestStatus::Failed,
                format!("Multiple credential issues: {:?}", credential_issues),
                Some(details),
            ))
        }
    }

    async fn test_unnecessary_services_disabled(
        &self,
        target: &mut Target,
    ) -> Result<(TestStatus, String, Option<String>)> {
        // Check for unnecessary services
        let unnecessary_services = [
            "telnet", "ftp", "rsh", "rlogin", "tftp", "finger", "chargen", "daytime", "echo",
            "discard",
        ];
        let mut running_unnecessary = Vec::new();

        let all_services = target
            .execute_command("systemctl list-units --type=service --state=running --no-pager")
            .await?;
        ...
        let details = format!(
            ...
            running_unnecessary,
            total_services,
            if gui_services.stdout.contains("no_gui") {
                "None"
            } else {
                "Present"
            }
        ...
        if running_unnecessary.is_empty() && total_services <= 25 {
            Ok((
                TestStatus::Passed,
                "Unnecessary services properly disabled".to_string(),
                Some(details),
            ))
        } else if running_unnecessary.len() <= 1 && total_services <= 35 {
            Ok((
                TestStatus::Warning,
                "Some unnecessary services may be running".to_string(),
                Some(details),
            ))
        } else {
            Ok((
                TestStatus::Failed,
                "Many unnecessary services running".to_string(),
                Some(details),
            ))
        }
    }

    async fn test_logging_configured(
        &self,
        target: &mut Target,
    ) -> Result<(TestStatus, String, Option<String>)> {
        ...
        // Check log rotation
        let logrotate_config = target
            .execute_command("ls -la /etc/logrotate.d/ | wc -l")
            .await?;

        // Check log files
        let log_files = target
            .execute_command("find /var/log -name '*.log' -type f | wc -l")
            .await?;
        ...
        // Check audit logging
        let audit_status = target
            .execute_command("systemctl is-active auditd 2>/dev/null || echo 'no_audit'")
            .await?;
        ...
        let details = format!(
            "Syslog: {}\nLogrotate configs: {}\nLog files: {}\nRemote logging: {}\nAudit: {}",
            syslog_status.stdout.trim(),
            logrotate_count,
            log_file_count,
            remote_log_count,
            audit_status.stdout.trim()
        );

        if logging_features.len() >= 4 {
            Ok((
                TestStatus::Passed,
                "Production logging properly configured".to_string(),
                Some(details),
            ))
        } else if logging_features.len() >= 3 {
            Ok((
                TestStatus::Warning,
                "Basic logging configured".to_string(),
                Some(details),
            ))
        } else {
            Ok((
                TestStatus::Failed,
                "Insufficient logging configuration".to_string(),
                Some(details),
            ))
        }
    }
    ...
    async fn test_monitoring_enabled(
        ...
        for tool in &monitoring_tools {
            let tool_check = target
                .execute_command(&format!("systemctl is-active {} 2>/dev/null", tool))
                .await?;
        ...
        let network_monitoring = target
            .execute_command(
                "which netstat 2>/dev/null && which ss 2>/dev/null || echo 'network_tools_missing'",
            )
            .await?;
        ...
        let details = format!(
            "Active monitoring: {:?}\nSystem tools: {}\nLog monitoring: {}\nNetwork tools: {}",
            active_monitoring,
            if system_stats.stdout.contains("basic_tools_missing") {
                "Missing"
            } else {
                "Available"
            },
            if log_monitoring.stdout.contains("no_log_monitoring") {
                "None"
            } else {
                "Active"
            },
            if network_monitoring.stdout.contains("network_tools_missing") {
                "Missing"
            } else {
                "Available"
            }
        ...
        if active_monitoring.len() >= 2 {
            Ok((
                TestStatus::Passed,
                "Comprehensive monitoring enabled".to_string(),
                Some(details),
            ))
        } else if !active_monitoring.is_empty()
            || !system_stats.stdout.contains("basic_tools_missing")
        {
            Ok((
                TestStatus::Warning,
                "Basic monitoring available".to_string(),
                Some(details),
            ))
        } else {
            Ok((
                TestStatus::Failed,
                "Insufficient monitoring configuration".to_string(),
                Some(details),
            ))
        }
    }

    async fn test_backup_systems_active(
        &self,
        target: &mut Target,
    ) -> Result<(TestStatus, String, Option<String>)> {
        ...
        for tool in &backup_tools {
            let tool_check = target
                .execute_command(&format!("which {} 2>/dev/null", tool))
                .await?;
        ...
        let backup_crons = target
            .execute_command("crontab -l 2>/dev/null | grep -E 'backup|rsync|tar' | wc -l")
            .await?;

        // Check for backup directories
        let backup_dirs = target
            .execute_command("find /backup /var/backup /home/backup -type d 2>/dev/null | wc -l")
            .await?;
        ...
        if backup_features.len() >= 3 {
            Ok((
                TestStatus::Passed,
                "Backup systems properly configured".to_string(),
                Some(details),
            ))
        } else if backup_features.len() >= 2 {
            Ok((
                TestStatus::Warning,
                "Basic backup systems available".to_string(),
                Some(details),
            ))
        } else {
            Ok((
                TestStatus::Failed,
                "Insufficient backup systems".to_string(),
                Some(details),
            ))
        }
    }

    async fn test_security_updates_enabled(
        &self,
        target: &mut Target,
    ) -> Result<(TestStatus, String, Option<String>)> {
        ...
        if update_features.len() >= 3 {
            Ok((
                TestStatus::Passed,
                "Security updates properly enabled".to_string(),
                Some(details),
            ))
        } else if update_features.len() >= 2 {
            Ok((
                TestStatus::Warning,
                "Basic update mechanisms available".to_string(),
                Some(details),
            ))
        } else {
            Ok((
                TestStatus::Failed,
                "Security updates not properly configured".to_string(),
                Some(details),
            ))
        }
    }

    async fn test_network_hardening(
        &self,
        target: &mut Target,
    ) -> Result<(TestStatus, String, Option<String>)> {
        ...
        let network_services = target
            .execute_command("netstat -tuln | grep -E ':21|:23|:25|:53|:69|:135|:139|:445' | wc -l")
            .await?;

        // Check TCP wrappers
        let tcp_wrappers = target
            .execute_command("ls -la /etc/hosts.allow /etc/hosts.deny 2>/dev/null | wc -l")
            .await?;
        ...
        let details = format!(
            ...
            if net_params.stdout.contains("no_sysctl") {
                "Not available"
            } else {
                "Available"
            },
        ...
        if hardening_features.len() >= 3 {
            Ok((
                TestStatus::Passed,
                "Network hardening properly applied".to_string(),
                Some(details),
            ))
        } else if hardening_features.len() >= 2 {
            Ok((
                TestStatus::Warning,
                "Basic network hardening applied".to_string(),
                Some(details),
            ))
        } else {
            Ok((
                TestStatus::Failed,
                "Insufficient network hardening".to_string(),
                Some(details),
            ))
        }
    }

    async fn test_filesystem_hardening(
        &self,
        target: &mut Target,
    ) -> Result<(TestStatus, String, Option<String>)> {
        // Check mount options
        let secure_mounts = target
            .execute_command("mount | grep -E 'nodev|nosuid|noexec' | wc -l")
            .await?;
        ...
        let world_writable = target
            .execute_command("find /etc /usr/bin /usr/sbin -type f -perm -002 2>/dev/null | wc -l")
            .await?;
        ...
        if hardening_features.len() >= 3 {
            Ok((
                TestStatus::Passed,
                "Filesystem hardening properly applied".to_string(),
                Some(details),
            ))
        } else if hardening_features.len() >= 2 {
            Ok((
                TestStatus::Warning,
                "Basic filesystem hardening applied".to_string(),
                Some(details),
            ))
        } else {
            Ok((
                TestStatus::Failed,
                "Insufficient filesystem hardening".to_string(),
                Some(details),
            ))
        }
    }
