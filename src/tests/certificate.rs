...
impl CertificateTests {
    async fn test_x509_validation(
        &self,
        target: &mut Target,
    ) -> Result<(TestStatus, String, Option<String>)> {
        // Check OpenSSL availability
        let openssl_check = target
            .execute_command("openssl version 2>/dev/null || echo 'no_openssl'")
            .await?;

        if openssl_check.stdout.contains("no_openssl") {
            return Ok((
                TestStatus::Skipped,
                "OpenSSL not available for certificate validation".to_string(),
                None,
            ));
        }
        ...
        if validation_features.len() >= 2 {
            Ok((
                TestStatus::Passed,
                "X.509 validation infrastructure ready".to_string(),
                Some(details),
            ))
        } else if !validation_features.is_empty() {
            Ok((
                TestStatus::Warning,
                "Basic X.509 validation available".to_string(),
                Some(details),
            ))
        } else {
            Ok((
                TestStatus::Failed,
                "X.509 validation not available".to_string(),
                Some(details),
            ))
        }
    }

    async fn test_pki_infrastructure(
        &self,
        target: &mut Target,
    ) -> Result<(TestStatus, String, Option<String>)> {
        ...
        let pki_dirs = target
            .execute_command(
                "ls -la /etc/pki /usr/share/ca-certificates /etc/ssl 2>/dev/null | grep ^d | wc -l",
            )
            .await?;
        ...
        if pki_features.len() >= 3 {
            Ok((
                TestStatus::Passed,
                "PKI infrastructure complete".to_string(),
                Some(details),
            ))
        } else if pki_features.len() >= 2 {
            Ok((
                TestStatus::Warning,
                "Basic PKI infrastructure".to_string(),
                Some(details),
            ))
        } else {
            Ok((
                TestStatus::Failed,
                "Insufficient PKI infrastructure".to_string(),
                Some(details),
            ))
        }
    }
    ...
    async fn test_certificate_expiration(
        ...
        if expiry_features.len() >= 3 {
            Ok((
                TestStatus::Passed,
                "Certificate expiration monitoring complete".to_string(),
                Some(details),
            ))
        } else if expiry_features.len() >= 2 {
            Ok((
                TestStatus::Warning,
                "Basic expiration monitoring".to_string(),
                Some(details),
            ))
        } else {
            Ok((
                TestStatus::Failed,
                "No certificate expiration monitoring".to_string(),
                Some(details),
            ))
        }
    }
    ...
    async fn test_certificate_chain(
        ...
        let chain_files = target
            .execute_command(
                "find /etc/ssl -name '*chain*' -o -name '*intermediate*' 2>/dev/null | wc -l",
            )
            .await?;
        ...
        if chain_features.len() >= 2 {
            Ok((
                TestStatus::Passed,
                "Certificate chain validation working".to_string(),
                Some(details),
            ))
        } else if !chain_features.is_empty() {
            Ok((
                TestStatus::Warning,
                "Basic chain validation".to_string(),
                Some(details),
            ))
        } else {
            Ok((
                TestStatus::Failed,
                "Certificate chain validation not working".to_string(),
                Some(details),
            ))
        }
    }
    ...
    async fn test_certificate_revocation(
        ...
        // Check for CRL files
        let crl_files = target
            .execute_command("find /etc/ssl -name '*.crl' 2>/dev/null | wc -l")
            .await?;

        // Check OCSP support
        let ocsp_check = target
            .execute_command("openssl ocsp -help 2>/dev/null | head -1 || echo 'no_ocsp'")
            .await?;
        ...
        if revocation_features.len() >= 2 {
            Ok((
                TestStatus::Passed,
                "Certificate revocation mechanisms active".to_string(),
                Some(details),
            ))
        } else if !revocation_features.is_empty() {
            Ok((
                TestStatus::Warning,
                "Basic revocation support".to_string(),
                Some(details),
            ))
        } else {
            Ok((
                TestStatus::Failed,
                "No certificate revocation checking".to_string(),
                Some(details),
            ))
        }
    }

    async fn test_secure_cert_storage(
        &self,
        target: &mut Target,
    ) -> Result<(TestStatus, String, Option<String>)> {
        ...
        if storage_features.len() >= 2 {
            Ok((
                TestStatus::Passed,
                "Certificate storage security good".to_string(),
                Some(details),
            ))
        } else if !storage_features.is_empty() {
            Ok((
                TestStatus::Warning,
                "Basic storage security".to_string(),
                Some(details),
            ))
        } else {
            Ok((
                TestStatus::Failed,
                "Poor certificate storage security".to_string(),
                Some(details),
            ))
        }
    }

    async fn test_ca_cert_management(
        &self,
        target: &mut Target,
    ) -> Result<(TestStatus, String, Option<String>)> {
        ...
        if ca_features.len() >= 3 {
            Ok((
                TestStatus::Passed,
                "CA certificate management complete".to_string(),
                Some(details),
            ))
        } else if ca_features.len() >= 2 {
            Ok((
                TestStatus::Warning,
                "Basic CA management".to_string(),
                Some(details),
            ))
        } else {
            Ok((
                TestStatus::Failed,
                "Poor CA certificate management".to_string(),
                Some(details),
            ))
        }
    }

    async fn test_tls_cert_validation(
        &self,
        target: &mut Target,
    ) -> Result<(TestStatus, String, Option<String>)> {
        // Check for TLS services
        let tls_services = target
            .execute_command("netstat -tuln | grep ':443\\|:993\\|:995\\|:636' | wc -l")
            .await?;
        ...
        if tls_features.len() >= 2 {
            Ok((
                TestStatus::Passed,
                "TLS certificate validation working".to_string(),
                Some(details),
            ))
        } else if !tls_features.is_empty() {
            Ok((
                TestStatus::Warning,
                "Basic TLS validation".to_string(),
                Some(details),
            ))
        } else {
            Ok((
                TestStatus::Failed,
                "TLS certificate validation not available".to_string(),
                Some(details),
            ))
        }
    }
    ...
    async fn test_certificate_rotation(
        ...
        if rotation_features.len() >= 2 {
            Ok((
                TestStatus::Passed,
                "Certificate rotation mechanisms active".to_string(),
                Some(details),
            ))
        } else if !rotation_features.is_empty() {
            Ok((
                TestStatus::Warning,
                "Basic rotation support".to_string(),
                Some(details),
            ))
        } else {
            Ok((
                TestStatus::Failed,
                "No certificate rotation mechanisms".to_string(),
                Some(details),
            ))
        }
    }
    ...
    async fn test_compliance_standards(
        ...
        let details = format!(
            ...
            if key_lengths.stdout.contains("no_key_info") {
                "Not available"
            } else {
                "Available"
            },
            if sig_algorithms.stdout.contains("no_sig_info") {
                "Not available"
            } else {
                "Available"
            },
        ...
        if compliance_features.len() >= 3 {
            Ok((
                TestStatus::Passed,
                "Certificate compliance standards met".to_string(),
                Some(details),
            ))
        } else if compliance_features.len() >= 2 {
            Ok((
                TestStatus::Warning,
                "Basic compliance standards".to_string(),
                Some(details),
            ))
        } else {
            Ok((
                TestStatus::Failed,
                "Certificate compliance standards not met".to_string(),
                Some(details),
            ))
        }
    }
