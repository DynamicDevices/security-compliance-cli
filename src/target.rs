...
impl Target {
    ...
    async fn check_foundries_registration(&mut self) -> String {
        ...
        let device_creds = self.execute_command("ls -la /var/sota/device-creds* 2>/dev/null | wc -l").await.unwrap_or_default();
        let has_device_creds = device_creds.stdout.trim().parse::<i32>().unwrap_or(0) > 0;
        
        // Check for device UUID/ID
        let device_uuid = self.execute_command("cat /var/sota/device-uuid 2>/dev/null || echo 'not_found'").await.unwrap_or_default();
        let has_device_uuid = !device_uuid.stdout.contains("not_found") && !device_uuid.stdout.trim().is_empty();
        
        // Check for aktualizr-lite service (OTA client)
        let aktualizr_status = self.execute_command("systemctl is-active aktualizr-lite 2>/dev/null || echo 'inactive'").await.unwrap_or_default();
        let aktualizr_active = aktualizr_status.stdout.trim() == "active";
        
        // Check for device registration in aktualizr config
        let aktualizr_config = self.execute_command("cat /var/sota/sota.toml 2>/dev/null | grep -E 'device_id|server' | head -2").await.unwrap_or_default();
        let has_server_config = aktualizr_config.stdout.contains("server") || aktualizr_config.stdout.contains("device_id");
    ...
    async fn check_wireguard_status(&mut self) -> String {
        ...
        let wg_module = self.execute_command("lsmod | grep wireguard").await.unwrap_or_default();
        let module_loaded = !wg_module.stdout.trim().is_empty();
        
        // Check for WireGuard interfaces
        let wg_interfaces = self.execute_command("ip link show type wireguard 2>/dev/null | grep -o 'wg[0-9]*' | head -5").await.unwrap_or_default();
        let interfaces: Vec<&str> = wg_interfaces.stdout.lines().filter(|line| !line.trim().is_empty()).collect();
        
        // Check WireGuard service status
        let wg_service = self.execute_command("systemctl is-active wg-quick@* 2>/dev/null | grep -v 'inactive' | head -1").await.unwrap_or_default();
        let service_active = wg_service.stdout.trim() == "active";
        
        // Check for WireGuard configuration files
        let wg_configs = self.execute_command("ls /etc/wireguard/*.conf 2>/dev/null | wc -l").await.unwrap_or_default();
        let config_count = wg_configs.stdout.trim().parse::<i32>().unwrap_or(0);
        
        // Check active connections
        let wg_show = self.execute_command("wg show 2>/dev/null | grep -E 'interface|peer' | wc -l").await.unwrap_or_default();
        ...
        if active_connections > 0 {
            status_parts.push(format!("{} Active Connection(s)", active_connections / 2)); // Divide by 2 as each connection shows interface + peer
        }
    ...
    async fn get_disk_usage(&mut self) -> String {
        ...
        let df_output = self.execute_command("df -h | grep -E '^/dev|^overlay|^tmpfs' | head -10").await.unwrap_or_default();
        ...
        for line in df_output.stdout.lines() {
            ...
            if parts.len() >= 6 {
                ...
                if mount_point == "/" {
                    disk_info.insert(0, format!("Root: {} total, {} used ({})", size, used, use_percent));
                    root_found = true;
                } else if mount_point == "/var" || mount_point == "/boot" {
                    disk_info.push(format!("{}: {} total, {} used ({})", mount_point, size, used, use_percent));
                }
            }
        }
        ...
        if !root_found {
            let root_df = self.execute_command("df -h / 2>/dev/null | tail -1").await.unwrap_or_default();
            if !root_df.stdout.trim().is_empty() {
                let parts: Vec<&str> = root_df.stdout.split_whitespace().collect();
                if parts.len() >= 5 {
                    disk_info.insert(0, format!("Root: {} total, {} used ({})", parts[1], parts[2], parts[4]));
                }
            }
        }
    ...
    async fn get_cpu_info(&mut self) -> String {
        ...
        let cpu_model = self.execute_command("cat /proc/cpuinfo | grep 'model name' | head -1 | cut -d':' -f2 | xargs").await.unwrap_or_default();
        let cpu_arch = self.execute_command("uname -m").await.unwrap_or_default();
        
        // Get CPU frequencies
        let cpu_freq_current = self.execute_command("cat /proc/cpuinfo | grep 'cpu MHz' | head -1 | cut -d':' -f2 | xargs").await.unwrap_or_default();
        let cpu_freq_max = self.execute_command("cat /sys/devices/system/cpu/cpu0/cpufreq/cpuinfo_max_freq 2>/dev/null | head -1").await.unwrap_or_default();
        let cpu_freq_min = self.execute_command("cat /sys/devices/system/cpu/cpu0/cpufreq/cpuinfo_min_freq 2>/dev/null | head -1").await.unwrap_or_default();
    ...
    async fn get_memory_usage(&mut self) -> String {
        ...
        let meminfo = self.execute_command("cat /proc/meminfo | grep -E '^MemTotal|^MemFree|^MemAvailable|^Buffers|^Cached'").await.unwrap_or_default();
        ...
        let available_kb = if mem_available_kb > 0 { mem_available_kb } else { mem_free_kb };
        ...
        if total_mb > 1024 {
            ...
            format!("{:.1} GB total, {:.1} GB used ({:.1}%)", total_gb, used_gb, usage_percent)
        } else {
            format!("{} MB total, {} MB used ({:.1}%)", total_mb, used_mb, usage_percent)
        }
    }

    async fn get_power_governor(&mut self) -> String {
        // Check CPU frequency governor
        let governor = self.execute_command("cat /sys/devices/system/cpu/cpu0/cpufreq/scaling_governor 2>/dev/null").await.unwrap_or_default();
        
        if governor.stdout.trim().is_empty() {
            // Check if cpufreq is available at all
            let cpufreq_available = self.execute_command("ls /sys/devices/system/cpu/cpu0/cpufreq/ 2>/dev/null | wc -l").await.unwrap_or_default();
            ...
            if !has_cpufreq {
                ...
                let cpu_model = self.execute_command("cat /proc/cpuinfo | grep -i 'model\\|processor' | head -1").await.unwrap_or_default();
                if cpu_model.stdout.to_lowercase().contains("arm") || cpu_model.stdout.to_lowercase().contains("cortex") {
        ...
        let available_governors = self.execute_command("cat /sys/devices/system/cpu/cpu0/cpufreq/scaling_available_governors 2>/dev/null").await.unwrap_or_default();
        
        // Get current CPU frequency
        let current_freq = self.execute_command("cat /sys/devices/system/cpu/cpu0/cpufreq/scaling_cur_freq 2>/dev/null").await.unwrap_or_default();
        ...
        if !available_governors.stdout.trim().is_empty() {
            let available: Vec<&str> = available_governors.stdout.trim().split_whitespace().collect();
            if available.len() > 1 {
